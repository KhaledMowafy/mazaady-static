import React from 'react'
import Image from 'next/image'
import Links from '../atoms/Links'
import Button from '../atoms/Button'
function Nav() {
  return (
    <div className="w-full fixed top-0 bg-white flex py-3 px-8 justify-between">
    <div className="flex w-4/12 justify-around items-center">
        <Image src="./logoQR.svg" width={108} height={43} alt="logo"/>
        <Links className="font-bold text-pink">Home</Links>
        <Links>Blogs</Links>
        <Links>Gifts</Links>
    </div>
    <div className="flex w-5/12 justify-around items-center">
        <Image className="cursor-pointer" src="./search-normal.svg" width={24} height={24} alt="logo"/>
        <Image  src="./smalline.svg" width={2} height={38} alt="logo"/>
        <Image className="cursor-pointer" src="./notification.svg" width={24} height={24} alt="logo"/>
        <Image src="./smalline.svg" width={2} height={38} alt="logo"/>
        <Image className="cursor-pointer" src="./prof.svg" width={40} height={40} alt="logo"/>
        <div className="w-2/6">
        <Button className="p-2">Add New Product</Button>
        </div>
        <div className="flex justify-between items-center w-1/6">
        <Image src="./global.svg" width={24} height={24} alt="logo" className="cursor-pointer"/>
        <Image src="./bigline.svg" width={2} height={22} alt="logo" />
        <p className="text-lg font-bold cursor-pointer">EN</p>
        </div>
    </div>
    </div>
  )
}

export default Nav
